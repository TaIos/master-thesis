...
\begin{listing}[h!]
    ...
    \end{cminted}
    \cprotect\caption{Unsuccessful computation submission response.}
...
paintings together with their flow and evaluation function $\pi$, see eq.~\ref{eq:objective}, in the format that is accepted by
mXparser\footnotemark[4].
An example of submission without specifying the instance is in the appendix in listing~\ref{lst:computation-submission-manual}.
...
\begin{listing}[h!]
    ...
    \end{cminted}
    \cprotect\caption[Example of computation submission with instance name.]{Example of computation submission of random\_10 instance using \verb|curl|\footnotemark[2] to a server running on \verb|localhost:9000|.}
